...
MODEL_PROFILES=(--profile qwen-fast --profile qwen-quality --profile deepseek --profile deepseek-32b --profile qwen-max --profile comfy --profile "$MATXA_PROFILE")
ALL_PROFILES=(--profile webui --profile qwen-fast --profile qwen-quality --profile deepseek --profile deepseek-32b --profile qwen-max --profile comfy --profile "$MATXA_PROFILE")
...
MODEL_SERVICES=(vllm-fast vllm-quality vllm-deepseek vllm-deepseek32b vllm-qwen32b comfyui)
COMPOSE_SERVICES=(postgres litellm docker-socket-proxy model-switcher open-webui admin-panel matxa-backend-cuda matxa-backend-cpu catotron-cpu matxa-adapter vllm-fast vllm-quality vllm-deepseek vllm-deepseek32b vllm-qwen32b comfyui)
